...
public class VampireLogic
{
    ...
    private const int HeroCfgId = 101001;
    ...
    public void Tick(int absFrame, IList<VsInput> inputs)
    {
        ...
        // 5. 自爆判定：接触玩家则敌人自爆死亡（玩家不扣血）
        for (int e = 0; e < Enemies.Count; e++)
            EnemySuicide(Enemies[e]);

        // 6. 集中清扫：回收死亡敌人回池并移除（倒序 RemoveAt 保持存活者相对顺序，结果确定）
    ...
    /// <summary>敌人向最近存活玩家匀速移动，并更新面朝方向</summary>
    ...
    /// <summary>战斗结束清理逻辑层资源：存活敌人全部回收回池，复用跨局</summary>
    public void Dispose()
    {
        for (int i = 0; i < Enemies.Count; i++)
            CoreMgr.ClassPool.Recycle(Enemies[i]);
